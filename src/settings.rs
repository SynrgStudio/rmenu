...
impl RmenuConfig {
    pub fn load(path: Option<&Path>) -> io::Result<Self> {
        ...
        if !config_path.exists() {
            ...
            let save_path = path.map_or_else(
                || Self::default_config_path().unwrap_or_else(|_| PathBuf::from("config.ini")),
                |p| p.to_path_buf(),
            );
    ...
    fn to_string(&self) -> String {
        ...
        s.push_str(&format!(
            "selected_background = #{:02X}{:02X}{:02X}\n",
        ...
        s.push_str(&format!(
            "selected_foreground = #{:02X}{:02X}{:02X}\n",
        ...
        s.push_str(
            "# Los siguientes valores se usan si default_layout es 'custom' o no está definido,\n",
        );
        ...
        s.push_str(&format!(
            "source_boost_history = {}\n",
            self.launcher.source_boost_history
        ));
        s.push_str(&format!(
            "source_boost_start_menu = {}\n",
    ...
    fn apply_properties(
        config: &mut RmenuConfig,
        properties: HashMap<String, HashMap<String, String>>,
    ) -> io::Result<()> {
        if let Some(colors_props) = properties.get("Colors") {
            if let Some(val) = colors_props.get("background") {
                config.colors.background = parse_color_hex(val)?;
            }
            if let Some(val) = colors_props.get("foreground") {
                config.colors.foreground = parse_color_hex(val)?;
            }
            if let Some(val) = colors_props.get("selected_background") {
                config.colors.selected_background = parse_color_hex(val)?;
            }
            if let Some(val) = colors_props.get("selected_foreground") {
                config.colors.selected_foreground = parse_color_hex(val)?;
            }
            if let Some(val) = colors_props.get("border") {
                config.colors.border = parse_color_hex(val)?;
            }
        }

        if let Some(dim_props) = properties.get("Dimensions") {
            if let Some(val) = dim_props.get("default_layout") {
                config.dimensions.default_layout = Some(val.clone());
            }
            if let Some(val) = dim_props.get("width_percent") {
                config.dimensions.width_percent = val.parse().ok();
            }
            if let Some(val) = dim_props.get("max_width") {
                config.dimensions.max_width = val.parse().ok();
            }
            if let Some(val) = dim_props.get("height") {
                config.dimensions.height = val.parse().unwrap_or(config.dimensions.height);
            }
            if let Some(val) = dim_props.get("item_height") {
                config.dimensions.item_height =
                    val.parse().unwrap_or(config.dimensions.item_height);
            }
            if let Some(val) = dim_props.get("x_position") {
                config.dimensions.x_position = Some(val.clone());
            }
            if let Some(val) = dim_props.get("y_position") {
                config.dimensions.y_position = Some(val.clone());
            }
            if let Some(val) = dim_props.get("padding") {
                config.dimensions.padding = val.parse().unwrap_or(config.dimensions.padding);
            }
            if let Some(val) = dim_props.get("border_width") {
                config.dimensions.border_width =
                    val.parse().unwrap_or(config.dimensions.border_width);
            }
        }

        if let Some(font_props) = properties.get("Font") {
            if let Some(val) = font_props.get("name") {
                config.font.name = Some(val.clone());
            }
            if let Some(val) = font_props.get("size") {
                config.font.size = val.parse().unwrap_or(config.font.size);
            }
            if let Some(val) = font_props.get("weight") {
                config.font.weight = val.parse().unwrap_or(config.font.weight);
            }
        }

        if let Some(behavior_props) = properties.get("Behavior") {
            if let Some(val) = behavior_props.get("case_sensitive") {
                config.behavior.case_sensitive =
                    val.parse().unwrap_or(config.behavior.case_sensitive);
            }
            if let Some(val) = behavior_props.get("instant_selection") {
                config.behavior.instant_selection =
                    val.parse().unwrap_or(config.behavior.instant_selection);
            }
            if let Some(val) = behavior_props.get("max_items") {
                config.behavior.max_items = val.parse().unwrap_or(config.behavior.max_items);
            }
            if let Some(val) = behavior_props.get("element_delimiter") {
                config.behavior.element_delimiter = val
                    .chars()
                    .next()
                    .unwrap_or(config.behavior.element_delimiter);
            }
        ...
        if let Some(launcher_props) = properties.get("Launcher") {
            if let Some(val) = launcher_props.get("launcher_mode_default") {
                config.launcher.launcher_mode_default =
                    val.parse().unwrap_or(config.launcher.launcher_mode_default);
            }
            if let Some(val) = launcher_props.get("enable_history") {
                config.launcher.enable_history =
                    val.parse().unwrap_or(config.launcher.enable_history);
            }
            if let Some(val) = launcher_props.get("enable_start_menu") {
                config.launcher.enable_start_menu =
                    val.parse().unwrap_or(config.launcher.enable_start_menu);
            }
            if let Some(val) = launcher_props.get("enable_path") {
                config.launcher.enable_path = val.parse().unwrap_or(config.launcher.enable_path);
            }
            if let Some(val) = launcher_props.get("history_max_items") {
                config.launcher.history_max_items =
                    val.parse().unwrap_or(config.launcher.history_max_items);
            }
            if let Some(val) = launcher_props.get("source_boost_history") {
                config.launcher.source_boost_history =
                    val.parse().unwrap_or(config.launcher.source_boost_history);
            }
            if let Some(val) = launcher_props.get("source_boost_start_menu") {
                config.launcher.source_boost_start_menu = val
                    .parse()
                    .unwrap_or(config.launcher.source_boost_start_menu);
            }
            if let Some(val) = launcher_props.get("source_boost_path") {
                config.launcher.source_boost_path =
                    val.parse().unwrap_or(config.launcher.source_boost_path);
            }
        ...
        if let Some(modules_props) = properties.get("Modules") {
            if let Some(val) = modules_props.get("provider_total_budget_ms") {
                config.modules.provider_total_budget_ms = val
                    .parse()
                    .unwrap_or(config.modules.provider_total_budget_ms);
            }
            if let Some(val) = modules_props.get("provider_timeout_ms") {
                config.modules.provider_timeout_ms =
                    val.parse().unwrap_or(config.modules.provider_timeout_ms);
            }
            if let Some(val) = modules_props.get("max_items_per_provider_host") {
                config.modules.max_items_per_provider_host = val
                    .parse()
                    .unwrap_or(config.modules.max_items_per_provider_host);
            }
            if let Some(val) = modules_props.get("dedupe_source_priority") {
                config.modules.dedupe_source_priority =
                    match val.trim().to_ascii_lowercase().as_str() {
                        "provider_first" => DedupeSourcePriority::ProviderFirst,
                        _ => DedupeSourcePriority::CoreFirst,
                    };
            }
            if let Some(val) = modules_props.get("host_restart_backoff_ms") {
                config.modules.host_restart_backoff_ms = val
                    .parse()
                    .unwrap_or(config.modules.host_restart_backoff_ms);
            }
            if let Some(val) = modules_props.get("max_ipc_payload_bytes") {
                config.modules.max_ipc_payload_bytes =
                    val.parse().unwrap_or(config.modules.max_ipc_payload_bytes);
            }
        }
        Ok(())
    }
        ...
        dirs::config_dir()
            ...
            .ok_or_else(|| {
                io::Error::new(
                    ...
                    "No se pudo encontrar el directorio de configuración del usuario",
                )
            })
    }

    pub fn apply_cli_overrides(&mut self, cmd_opts: &CmdOptions) {
        if let Some(val) = cmd_opts.cli_width_percent {
            self.dimensions.width_percent = Some(val);
        }
        if let Some(val) = cmd_opts.cli_max_width {
            self.dimensions.max_width = Some(val);
        }
        if let Some(val) = cmd_opts.cli_height {
            self.dimensions.height = val;
        }
        if let Some(val) = cmd_opts.cli_item_height {
            self.dimensions.item_height = val;
        }
        if let Some(val) = &cmd_opts.cli_x_pos {
            self.dimensions.x_position = Some(val.clone());
        }
        if let Some(val) = &cmd_opts.cli_y_pos {
            self.dimensions.y_position = Some(val.clone());
        }
        if let Some(val) = cmd_opts.cli_padding {
            self.dimensions.padding = val;
        }
        if let Some(val) = cmd_opts.cli_border_width {
            self.dimensions.border_width = val;
        }
    }
}
...
fn parse_color_hex(hex: &str) -> io::Result<COLORREF> {
    ...
    if hex.len() != 6 {
        return Err(io::Error::new(
            ...
            "Color hex debe tener 6 dígitos",
        ));
    }
    let r = u8::from_str_radix(&hex[0..2], 16)
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "Componente R inválido"))?;
    let g = u8::from_str_radix(&hex[2..4], 16)
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "Componente G inválido"))?;
    let b = u8::from_str_radix(&hex[4..6], 16)
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "Componente B inválido"))?;
...
pub fn parse_args() -> CmdOptions {
    ...
    while i < args.len() {
        ...
        match arg.as_str() {
            "-e" | "--elements" => {
                if i + 1 < args.len() {
                    options.elements_str = Some(args[i + 1].clone());
                    i += 1;
                }
            }
            "-p" | "--prompt" => {
            ...
            "--debug-ranking" => {
                if i + 1 < args.len() {
                    options.debug_ranking = Some(args[i + 1].clone());
                    i += 1;
                }
            }
            "--metrics" => {
            ...
            "--layout" => {
                if i + 1 < args.len() {
                    options.layout = Some(args[i + 1].to_lowercase().clone());
                    i += 1;
                }
            }
            "--x-pos" => {
                if i + 1 < args.len() {
                    options.cli_x_pos = Some(args[i + 1].clone());
                    i += 1;
                }
            }
            "--y-pos" => {
                if i + 1 < args.len() {
                    options.cli_y_pos = Some(args[i + 1].clone());
                    i += 1;
                }
            }
            "--width-percent" => {
                if i + 1 < args.len() {
                    options.cli_width_percent = args[i + 1].parse().ok();
                    i += 1;
                }
            }
            "--max-width" => {
                if i + 1 < args.len() {
                    options.cli_max_width = args[i + 1].parse().ok();
                    i += 1;
                }
            }
            "--height" => {
                if i + 1 < args.len() {
                    options.cli_height = args[i + 1].parse().ok();
                    i += 1;
                }
            }
            "--item-height" => {
                if i + 1 < args.len() {
                    options.cli_item_height = args[i + 1].parse().ok();
                    i += 1;
                }
            }
            "--padding" => {
                if i + 1 < args.len() {
                    options.cli_padding = args[i + 1].parse().ok();
                    i += 1;
                }
            }
            "--border-width" => {
                if i + 1 < args.len() {
                    options.cli_border_width = args[i + 1].parse().ok();
                    i += 1;
                }
            }
...
mod tests {
    ...
    fn invalid_module_config_values_fall_back_to_safe_defaults() {
        let defaults = RmenuConfig::default();
        let parsed = RmenuConfig::parse_config(
            r#"
[Modules]
provider_total_budget_ms = not-a-number
provider_timeout_ms = nope
max_items_per_provider_host = invalid
dedupe_source_priority = unsupported
host_restart_backoff_ms = bad
max_ipc_payload_bytes = also-bad
"#,
        )
        .expect("invalid module values should not reject the whole config");

        assert_eq!(
            parsed.modules.provider_total_budget_ms,
            defaults.modules.provider_total_budget_ms
        );
        assert_eq!(
            parsed.modules.provider_timeout_ms,
            defaults.modules.provider_timeout_ms
        );
        assert_eq!(
            parsed.modules.max_items_per_provider_host,
            defaults.modules.max_items_per_provider_host
        );
        assert_eq!(
            parsed.modules.dedupe_source_priority,
            DedupeSourcePriority::CoreFirst
        );
        assert_eq!(
            parsed.modules.host_restart_backoff_ms,
            defaults.modules.host_restart_backoff_ms
        );
        assert_eq!(
    ...
    #[test]
    fn module_config_values_parse_when_valid() {
        let parsed = RmenuConfig::parse_config(
            r#"
[Modules]
provider_total_budget_ms = 50
provider_timeout_ms = 2000
max_items_per_provider_host = 12
dedupe_source_priority = provider_first
host_restart_backoff_ms = 900
max_ipc_payload_bytes = 131072
"#,
        )
        .expect("valid module values should parse");

        assert_eq!(parsed.modules.provider_total_budget_ms, 50);
        assert_eq!(parsed.modules.provider_timeout_ms, 2000);
        assert_eq!(parsed.modules.max_items_per_provider_host, 12);
        assert_eq!(
            parsed.modules.dedupe_source_priority,
            DedupeSourcePriority::ProviderFirst
        );
        assert_eq!(parsed.modules.host_restart_backoff_ms, 900);
        assert_eq!(parsed.modules.max_ipc_payload_bytes, 131072);
    }
}
...
pub fn print_help() {
    ...
    println!(
        "  -e, --elements <LIST>   Lista de elementos (delimitador en config.ini, defecto: ',')."
    );
    ...
    println!(
        "  --debug-ranking <QUERY> Imprime ranking (fuzzy + source boost) para depuración y sale."
    );
    ...
    println!(
        "                            Si es 'custom' o se omite, se usan los valores detallados."
    );
    ...
    println!("");
    println!(
        "Para más detalles sobre colores, fuentes y comportamiento, ver el archivo config.ini."
    );
